...
InstructionsUI::InstructionsUI(QWidget *parent) : QDialog(parent) { // 挖地雷说明页
    ...
    titleLabel->setText(u"挖地雷玩法说明"_s);
    titleLabel->setAlignment(QFlags(AlignCenter));
    titleLabel->setStyleSheet(u"color: #44CC86; font-size: 48px; font-weight: bold;"_s);

    auto label1 = make_unique<QLabel>(this);
    label1->setText(uR"(挖地雷是一款经典的单人游戏，是许多人童年的回忆。
...
七、排行榜
    ...
    label1->setStyleSheet(u"color: #333; font-size: 16px;"_s);

    auto label2 = make_unique<QLabel>(this);
    label2->setText(u"以上就是挖地雷游戏的基本玩法说明。祝你游戏愉快，小心地雷！"_s);
    label2->setWordWrap(true);
    label2->setAlignment(QFlags(AlignCenter));
    label2->setStyleSheet(u"color: #555; font-size: 24px; font-weight: bold;"_s);

    auto layout = make_unique<QVBoxLayout>(this);
    layout->addWidget(label1.release());
    layout->addWidget(label2.release());

    auto scrollArea = make_unique<QScrollArea>(this);
    scrollArea->setWidget(new QWidget(this));
    scrollArea->widget()->setLayout(layout.release());
    ...
    auto mainLayout = make_unique<QVBoxLayout>(this);
